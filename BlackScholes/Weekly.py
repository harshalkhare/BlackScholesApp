...
# Time Left to Expiry
expiry_date = datetime.strptime(WE3, "%d-%m-%Y")
...
strike = get_atm_strike_by_expiry(Stock, WE2) # StrikePrice
strike_str = str(int(strike)) if strike == int(strike) else str(strike) #Converts Decimal and string as required

callstrike = f"{Stock}{WE}C{strike_str}" #IREDA250626C170
putstrike = f"{Stock}{WE}P{strike_str}" #IREDA250626P170
sigma = get_atm_iv_by_expiry(Stock, WE2) #0.362158
